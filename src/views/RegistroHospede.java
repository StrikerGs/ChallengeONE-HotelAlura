...
public class RegistroHospede extends JFrame {
	...
	// passar o id gerado pela inserção da reserva por um construtor
	/**
	...
	public RegistroHospede() {
		...
		// setEditable() passando o boleano false para não ser possivel mexer no valor mostrado
		...
		txtNreserva.addPropertyChangeListener(new PropertyChangeListener() {
			
			@Override
			public void propertyChange(PropertyChangeEvent evt) {
				
				// com esse método addPropertyChangeListener conseguimos mostrar o número da reserva
				mostrarNumeroDaReserva(reservaId);
		...
		btnsalvar.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(!txtNome.getText().isEmpty() && !txtSobrenome.getText().isEmpty() && txtDataN.getDate() != null && !txtTelefone.getText().isEmpty()) {
				
					// ao clicar no botão salvar, o evento do clique do mouse é ativado e chama o seguinte método:
					salvar(reservaId);
					System.out.println(hospede);
					// abrindo o menu do usuario após salvar e fechando a tela de registro
					MenuUsuario usuario = new MenuUsuario();
					usuario.setVisible(true);
					// setando o atributo encerrar para true para fehcar o jframe anterior
					setEncerrar(true);
					// dispose() fecha o jframe RegistroHospede
					dispose();
					...
					JOptionPane.showMessageDialog(null, "Você precisa preencher todos os dados.");
	...
	private void salvar(Reserva reserva) {
		...
		LocalDate dataNasc = txtDataN.getDate().toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
		...
		hospede.setReserva_id(reserva.getId());
		System.out.println(reserva);
		hospedeController = new HospedeController();
		hospedeController.salvar(hospede);
		JOptionPane.showMessageDialog(null, "Reserva feita e hospede cadastrado com sucesso. | Número da reserva: " + reserva.getId());
		
	}
	
	public void exportarId(Reserva reservaId) {
		
		this.reservaId = reservaId;
		
	}
	
	private void mostrarNumeroDaReserva(Reserva reserva) {
		
		txtNreserva.setText(reserva.getId().toString());
		
	}
	
	private void setEncerrar(boolean bool) {
