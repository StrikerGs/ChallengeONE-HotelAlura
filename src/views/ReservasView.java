		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {	
					...
					// método exportarId passando a reserva para que possamos pegar seu id
					registro.exportarId(reserva);
					registro.setVisible(true);
					// veficando se o método retorna true para fecharmos também esse jframe
					if(registro.getEncerrar()) {
	...
	//Código que permite movimentar a janela pela tela seguindo a posição de "x" e "y"	
	 ...
	 private void calcularValor() {
		 ...
		 if(txtDataE.getDate() != null && txtDataS.getDate() != null) {
			...
			Long diferencaDias = dataEntrada.until(dataSaida, ChronoUnit.DAYS);
			
			if(diferencaDias != 0) {
			
				reserva.setDataEntrada(dataEntrada);
				reserva.setDataSaida(dataSaida);
				reserva.setValor(diferencaDias);
	 ...
	 private void mostrarValor() {
		...
		if(reserva.getValor() != null) {
				...
				txtValor.setText(reserva.getValor().toPlainString());
	 ...
	 private void escolherFormaDePagamento() {
		 ...
		 System.out.println(reserva.getFormaDePagamento());
	 }
